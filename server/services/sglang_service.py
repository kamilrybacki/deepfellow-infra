...
class SglangService(Base2Service[InstalledInfo, DownloadedInfo]):
    ...
    def _add_custom_model(self, instance: str, model: CustomModel) -> None:
        ...
        self.models[instance][parsed.id] = SglangModel(hf_id=parsed.hf_id, size=parsed.size, custom=model.id, model_type=parsed.model_type)
    ...
    async def _download_model(self, stream: Stream[StreamChunk], model_id: str, model: SglangModel, model_dir: Path) -> Path | None:
    ...
    async def _download_model_or_set_progress(
        self, stream: Stream[StreamChunk], model_id: str, model: SglangModel, model_dir: Path
    ) -> Path:
        local_model_path: Path | None = None
