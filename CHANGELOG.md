...
- New `standard_proxy_timeout_seconds`/`DF_STANDARD_PROXY_TIMEOUT_SECONDS` setting (default 300) controls the timeout for outbound proxy HTTP requests to backend model/tool services (chat completions, embeddings, audio, images, rerank, MCP), previously hardcoded to aiohttp's unconfigurable 300-second default. Does not apply to custom-endpoint proxying, which has its own independent read timeout below.
- New `custom_endpoint_read_timeout_seconds`/`DF_CUSTOM_ENDPOINT_READ_TIMEOUT_SECONDS` setting (default 900) controls the idle-read timeout for custom-service proxy requests (doc-chunker etc.), previously hardcoded. Both settings above are live-editable via `/admin/config` and the WebUI settings page, not just `.env`.
